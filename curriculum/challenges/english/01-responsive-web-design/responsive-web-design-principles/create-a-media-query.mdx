...
```js
assert($("style").text().replace(/\s/g ,'').match(/@media\(max-height:800px\)/g));
```

Your `p` element should have a `font-size` of 10px when the device `height` is less than or equal to 800px.

```js
assert($("style").text().replace(/\s/g ,'').match(/@media\(max-height:800px\){p{font-size:10px;?}}/g));
```

Your `p` element should have an initial `font-size` of 20px when the device `height` is more than 800px.

```js
assert($("style").text().replace(/\s/g ,'').replace(/@media.*}/g, '').match(/p{font-size:20px;?}/g));
```
