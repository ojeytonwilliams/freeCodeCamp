...
```js
getUserInput => $.get(getUserInput('url') + '/_api/package.json').then(data => { var packJson = JSON.parse(data); assert.equal(packJson.dependencies.moment,"2.10.2", 'Wrong version of "moment". It should be 2.10.2'); }, xhr => { throw new Error(xhr.responseText); })
```
