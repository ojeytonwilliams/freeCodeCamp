...
```js
assert((function() { const initialState = store.getState(); return (Array.isArray(initialState) === true && DeepEqual(initialState, [0, 1, 2, 3, 4, 5])); })());
```

`removeItem` and `immutableReducer` both should be functions.

```js
assert(typeof removeItem === 'function' && typeof immutableReducer === 'function');
```
