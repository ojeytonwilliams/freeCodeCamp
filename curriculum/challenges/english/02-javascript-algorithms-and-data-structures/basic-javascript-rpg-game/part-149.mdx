...
```js
assert( pick.toString().replace(/\s/g, '').includes('text.innerText+="Right!Youwin20gold!";') && pick.toString().replace(/\s/g, '').includes('gold+=20;') && pick.toString().replace(/\s/g, '').includes('goldText.innerText=gold;') );
```
