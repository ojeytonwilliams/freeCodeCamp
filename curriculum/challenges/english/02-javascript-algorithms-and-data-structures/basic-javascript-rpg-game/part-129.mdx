...
```js
assert(isMonsterHit.toString().replace(/\s/g, '').includes('returnMath.random()>.2||health<20') || isMonsterHit.toString().replace(/\s/g, '').includes('returnMath.random()>0.2||health<20'));
```
